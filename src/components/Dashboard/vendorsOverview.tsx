...
import { vendorsOverviewTableData } from "@/pages/Dashboard/utils";
import { SidebarRoutes } from "@/types/routes";

function VendorsOverview(): JSX.Element {
  return (
    <div className="w-full py-3 border dark:border-dark rounded-xl md:rounded-2xl shadow-sm">
      <div className="flex justify-between mb-2 px-4">
        <p className="px-4 py-2 text-base font-medium">Vendor overview</p>
        <Link href={SidebarRoutes.vendors} color="primary">
          Vendors
        </Link>
      </div>
      <Table
        radius="none"
        removeWrapper
        aria-label="Vendors overview table"
        classNames={{
      ...
      >
        ...
        <TableBody>
          {vendorsOverviewTableData.map((data, index) => {
            const progressColorClass =
              data.progress.charAt(0) === "+" ? "text-success-500" : "text-red";
            return (
              <TableRow
                key={index}
                className={`${index === vendorsOverviewTableData.length - 1 ? "" : "border-b-1 dark:border-dark h-16"}`}
              >
                <TableCell>
